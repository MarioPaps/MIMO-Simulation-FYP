...
for trial=1:1
    ...
    %    gamma5= computegamma(beta(:,4*Nsc+1:5*Nsc),DODs(5,:),Fjvec,r_bar,K);
        ...
        down(trial,Nsc)=ctranspose(doppstar_weights)*(Rmai+Risi+Rnn)* (doppstar_weights);
    end
    
   % up=[up,ctranspose(doppstar_weights)*Rdes* (doppstar_weights)];
   % down=[down,ctranspose(doppstar_weights)*(Rmai+Risi+Rnn)* (doppstar_weights)];
%     SNIRout_doppstar= (ctranspose(doppstar_weights)*Rdes* (doppstar_weights))/ (ctranspose(doppstar_weights)*(Rnn)* (doppstar_weights));
%     SNIR_nsc(Nsc)=10*log10(real(SNIRout_doppstar));

end
%% 
 plot((1:4),10*log10(real(up(1,:)./down(1,:))),'-o');
 figure;
